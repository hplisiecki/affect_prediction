...
#                                 return_tensors="pt") for text in warriner['Word']]
# longest = 0
# for i in t:
...
class BertRegression(nn.Module):
    ...
    def __init__(self, dropout=0.2, hidden_dim=768):
        ...
        self.l_affect = nn.Linear(hidden_dim, hidden_dim)
        self.l_arousal = nn.Linear(hidden_dim, hidden_dim)
        self.l_dominance = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, input_id, mask):
        ...
        affect_all = self.l_affect(x) + x
        arousal_all = self.l_arousal(x) + x
        dominance_all = self.l_dominance(x) + x

        affect_all = self.relu(self.dropout(self.layer_norm(affect_all)))
        arousal_all = self.relu(self.dropout(self.layer_norm(arousal_all)))
        dominance_all = self.relu(self.dropout(self.layer_norm(dominance_all)))
...
print(corr_aro)
print(corr_dom)
torch.save(model.state_dict(), 'big_uncased_0_005.pt')
