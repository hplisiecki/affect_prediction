...
# remove left over files to avoid corrupted saves
...
if os.path.exists('models/best_corr_concreteness.pth'):
    os.remove('models/best_corr_concreteness.pth')
...
model.load_state_dict(torch.load('C:/Users/hplis/PycharmProjects/social_ai/models/final/best_corr_valence.pth'))
...
error = []
raters = []
for i, j, r in zip(pred_val, true_val, df_test['D.Rat.Sum']):
    if r < 200:
