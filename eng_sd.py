...
aoa['norm_aoa_sd'] = (aoa['Rating.SD'] - min(aoa['Rating.SD'])) / (max(aoa['Rating.SD']) - min(aoa['Rating.SD']))
aoa = aoa.dropna(subset=['norm_aoa_sd'])
...
esm['norm_concreteness_sd'] = (esm['Conc.SD'] - min(esm['Conc.SD'])) / (max(esm['Conc.SD']) - min(esm['Conc.SD']))
esm = esm.dropna(subset=['norm_concreteness_sd'])
...
aoa_sd = [temp[temp['Word'] == w]['norm_aoa_sd'].values[0] for w in list(a['Word'])]
a['norm_aoa_sd'] = aoa_sd
...
concreteness_sd = [temp[temp['Word'] == w]['norm_concreteness_sd'].values[0] for w in list(df_train['Word'])]
df_train['norm_concreteness_sd'] = concreteness_sd
...
concreteness_sd = [temp[temp['Word'] == w]['norm_concreteness_sd'].values[0] for w in list(df_test['Word'])]
df_test['norm_concreteness_sd'] = concreteness_sd
...
concreteness_sd = [temp[temp['Word'] == w]['norm_concreteness_sd'].values[0] for w in list(df_val['Word'])]
df_val['norm_concreteness_sd'] = concreteness_sd
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, df):
        ...
        self.labels_aoa_sd = df['norm_aoa_sd'].values.astype(float)
        self.labels_concreteness_sd = df['norm_concreteness_sd'].values.astype(float)
    ...
    def classes(self):
        return self.labels_valence, self.labels_arousal, self.labels_dominance, self.labels_aoa, self.labels_concreteness, self.labels_valence_sd, self.labels_arousal_sd, self.labels_dominance_sd, self.labels_aoa_sd, self.labels_concreteness_sd
...
class BertRegression(nn.Module):
    ...
    def __init__(self, dropout=0.1, hidden_dim_valence=1536, hidden_dim_arousal = 1536, hidden_dim_dominance = 1536):
        ...
        self.af_sd = nn.Linear(hidden_dim_valence, 1)
        self.ar_sd = nn.Linear(hidden_dim_valence, 1)
        self.do_sd = nn.Linear(hidden_dim_valence, 1)
        self.aoa_sd = nn.Linear(hidden_dim_valence, 1)
        self.co_sd = nn.Linear(hidden_dim_valence, 1)
    ...
    def forward(self, input_id1, mask1, input_id3, mask3):
        ...
        aoa_sd = self.sigmoid(self.aoa_sd(aoa_all))
        concreteness_sd = self.sigmoid(self.co_sd(concreteness_all))

        return affect, dominance, arousal, aoa, concreteness, affect_sd, dominance_sd, arousal_sd, aoa_sd, concreteness_sd
...
for epoch_num in range(epochs):
    ...
    for train_input1, train_input2, (valence, arousal, dominance, aoa, conc, v_sd, a_sd, d_sd, aoa_sd, conc_sd) in tqdm(train_dataloader):
        ...
        sd_label = torch.cat((v_sd, a_sd, d_sd, aoa_sd, conc_sd), dim=0).to(device)
        del valence, arousal, dominance, aoa, conc, v_sd, a_sd, d_sd, aoa_sd, conc_sd

        o1, o2, o3, o4, o5, o6, o7, o8, o9, o10 = model(input_id1, mask1, input_id2, mask2)
        del input_id1, input_id2, mask1, mask2

        output_a = torch.cat((o1, o2, o3, o4, o5), dim=0)
        del o1, o2, o3, o4, o5

        l1 = criterion1(output_a.float(), train_label.view(-1,1).float())
        del output_a, train_label

        output_b = torch.cat((o6, o7, o8, o9, o10), dim=0)
        del o6, o7, o8, o9, o10

        l2 = criterion1(output_b.float(), sd_label.view(-1,1).float())
    ...
    with torch.no_grad():
        for val_input1, val_input2, (val_valence, val_arousal, val_dominance, val_aoa, val_conc, v_sd, a_sd, d_sd, aoa_sd, conc_sd) in val_dataloader:
            ...
            val_output_a = torch.cat((o1, o2, o3, o4, o5), dim=0).to(device)
            l1 = criterion1(val_output_a.float(), val_label.view(-1,1).float())

            batch_loss = l1
            total_loss_val += batch_loss.item()

            output_b = torch.cat((o6, o7, o8, o9, o10), dim=0).to(device)
            sd_label = torch.cat((v_sd, a_sd, d_sd, aoa_sd, conc_sd), dim=0).to(device)
            l2 = criterion1(output_b.float(), sd_label.view(-1,1).float())

            batch_loss_sd = l2
            total_loss_sd += batch_loss_sd.item()

            o1, o2, o3, o4, o5 = o1.cpu().detach().view(-1).numpy(), o2.cpu().detach().view(-1).numpy(), o3.cpu().detach().view(-1).numpy(), o4.cpu().detach().view(-1).numpy(), o5.cpu().detach().view(-1).numpy()
            ...
            total_corr_valence += np.corrcoef(o1, val_valence)[0, 1]
            total_corr_arousal += np.corrcoef(o2, val_arousal)[0, 1]
            total_corr_dominance += np.corrcoef(o3, val_dominance)[0, 1]
            total_corr_aoa += np.corrcoef(o4, val_aoa)[0, 1]
            total_corr_conc += np.corrcoef(o5, val_conc)[0, 1]
...
def evaluate(model, test_data):
    ...
    with torch.no_grad():
        ...
        for test_input2, test_input3, (test_valence, test_arousal, test_dominance, test_aoa, test_conc, v_sd, a_sd, d_sd, aoa_sd, conc_sd) in test_dataloader:
            ...
            output1, output2, output3, output4, output5, o6, o7, o8, o9, o10  = model(input_id2, mask2, input_id3, mask3)
